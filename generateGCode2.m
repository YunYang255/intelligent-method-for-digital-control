%% G代码的生成
function generateGCode2(entities, inputFile)
...
function [x_d, y_d] = processEntity(e, x_d, y_d,deep, fid)
    if strcmp(e.name, 'LINE')
        ...
        if (round_x_d ~= round_x || round_y_d ~= round_y) % 判断断开
            ...
            fprintf(fid, 'G01 X%.3f Y%.3f ; 直线插补\n', e.line(1, 1)-x_d, e.line(1, 2)-y_d); % 直线插补去到直线起点坐标
            fprintf(fid, ' Z%.3f; 下刀\n', -2*deep);
        end
        fprintf(fid, 'G01 X%.3f Y%.3f ; 直线插补\n', e.line(1, 3)-x_d, e.line(1, 4)-y_d);
        x_d = e.line(1, 3);
        y_d = e.line(1, 4);
    ...
    elseif strcmp(e.name, 'CIRCLE')
        ...
        if distance > e.circle(1, 3)^2  % 圆与上一个轮廓脱离
            fprintf(fid, ' Z%.3f; 抬刀防止撞刀\n', 2*deep);
            fprintf(fid, 'G01 X%.3f Y%.3f ; 直线插补\n', e.circle(1, 1) + e.circle(1, 3)-x_d, e.circle(1, 2)-y_d); % 直线插补走到圆上
            fprintf(fid, ' Z%.3f; 下刀\n', -2*deep);
            fprintf(fid, 'G02 X%.3f Y%.3f I%.3f J%.3f; 圆\n', 0, 0, e.circle(1, 1) - e.circle(1, 1) - e.circle(1, 3), e.circle(1, 2) - e.circle(1, 2)); % 画整圆
            x_d = e.circle(1, 1) + e.circle(1, 3);
            y_d = e.circle(1, 2);
        else
            fprintf(fid, 'G02 X%.3f Y%.3f I%.3f J%.3f; 圆\n', 0, 0, e.circle(1, 1) - x_d, e.circle(1, 2) - y_d); % 画整圆
        end
...
function [x_d, y_d] = processEntity_with_S_and_F(e, x_d, y_d, s , f, deep,fid)
    ...
    if strcmp(e.name, 'LINE')
        ...
        fprintf(fid, 'G01 X%.3f Y%.3f F%.3f S%.3f M02 G%.0f T0%.0f; 直线插补\n', e.line(1, 3)-x_d, e.line(1, 4)-y_d,f,s ,conpensation ,tool_num );
        x_d = e.line(1, 3);
        y_d = e.line(1, 4);
    ...
    elseif strcmp(e.name, 'CIRCLE')
        ...
        if distance > e.circle(1, 3)^2  % 圆与上一个轮廓脱离
            ...
            fprintf(fid, ' Z%.3f; 下刀\n', -2*deep);
            fprintf(fid, 'G02 X%.3f Y%.3f I%.3f J%.3f F%.3f S%.3f M02 G%.0f T0%.0f ; 圆\n', 0, 0, e.circle(1, 1) - e.circle(1, 1) - e.circle(1, 3), e.circle(1, 2) - e.circle(1, 2),f,s ,conpensation ,tool_num); % 画整圆
            x_d = e.circle(1, 1) + e.circle(1, 3);
            y_d = e.circle(1, 2);
        else
            fprintf(fid, 'G02 X%.3f Y%.3f I%.3f J%.3f F%.3f S%.3f M02 G%.0f T0%.0f; 圆\n', 0, 0, e.circle(1, 1) - x_d, e.circle(1, 2) - y_d,f,s,conpensation ,tool_num); % 画整圆
        end
