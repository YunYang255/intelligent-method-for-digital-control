...
用DXFtool函数获取读入DXF文件的内容，DXFtool内部有function h = plot_line(line,col)等可以把起终点的坐标算出来并存到entities的line中。不仅是直线还有圆弧的圆心坐标，半径起终点的角度也可以输出到entities中。DXFtool内还有函数read_dxf(filename)可以将识别到的每一条线的线型识别出来并输出到

entities的name中。后续我们通过识别这个可以判断当下作图是需要画直线还是圆弧。
